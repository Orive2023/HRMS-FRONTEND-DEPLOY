...
const BalanceForm = ({formData,setFormData, setFormVisible, setToggle}) => {
  ...
  const {
    ...
    setDescription,
    setFile,
    updatedTotalCredit, setUpdatedTotalCredit, updatedTotalDebit, setUpdatedTotalDebit,
    genId,
    setGenId,
    Balance,
    setBalance,
    ...
    setDateError,
    recDelete,
    totalDebit, setTotalDebit,totalCredit, setTotalCredit
  } = StateBalance();
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "totalCredit") {
        setTotalCredit(parseFloat(value) || 0);
      }
    
      if (name === "totalDebit") {
        setTotalDebit(parseFloat(value) || 0);
      }
  ...
  const handleItemChange = (id, field, value) => {
    ...
    if (field === "debit") {
      const updatedTotalDebit = updatedItems.reduce((total, item) => {
        const itemAmount = parseFloat(item.debit);
    ...
    if (field === "totalDebit") {
      formDataWithUpdatedItems.totalDebit = value;
    }
    if (field === "totalCredit") {
      formDataWithUpdatedItems.totalCredit = value;
    }

    const formDataWithTotalDebit = {
      ...formData,
      totalDebit: updatedTotalDebit,
    };
    setFormData(formDataWithTotalDebit);
