...
const EditContraVoucher = () => {
  ...
  const loadContraVoucher = async () => {
    const result = await axios.get(`http://localhost:8093/contraVoucher/get/${id}`);
  ...
  const updateContraVoucher = async (e) => {
    e.preventDefault();
    await axios.put(`http://localhost:8093/contraVoucher/update/${id}`, contraVoucher);
        ...
        <div className="row mb-5">
          ...
          <div className="col-sm-2">
            <Button
              id="input-btn-cancel"
                  variant="outlined"
                  onClick={() => navigate("/account/contra-voucher")}
            >
              Back
            </Button>
          </div>
